...
ESC = unichr(pygame.K_ESCAPE); U_ARROW = unichr(pygame.K_UP); D_ARROW = unichr(pygame.K_DOWN); R_ARROW = unichr(pygame.K_RIGHT); L_ARROW = unichr(pygame.K_LEFT); SPACE = unichr(pygame.K_SPACE)
NUM0 = unichr(pygame.K_KP0);NUM1 = unichr(pygame.K_KP1);NUM2 = unichr(pygame.K_KP2);NUM3 = unichr(pygame.K_KP3);NUM4 = unichr(pygame.K_KP4);NUM5 = unichr(pygame.K_KP5);NUM6 = unichr(pygame.K_KP6);NUM7 = unichr(pygame.K_KP7);NUM8 = unichr(pygame.K_KP8);NUM9 = unichr(pygame.K_KP9);
...
def detectSide(static, dynamic):
    ...
    #Test what side that the object collided on
    ...
    #return the normal of that side (radians)
...
class Player(pygame.sprite.Sprite):
    ...
    def move(self, vect, sprites):#, portals):#rotation):
        ...
        # pos is used for calculations as a float provides more precision
        self.pos[0]+=vect[0] * 2.0 
        self.rect.x = self.pos[0]
        collisionList = pygame.sprite.spritecollide(self, sprites, False)
        #calculate movements with each object that the player collided with
        for sprite in collisionList:
            if sprite is self or type(sprite) is Bullet: continue
            #test if travelling up
            elif vect[0] > 0:
                self.rect.right = sprite.rect.left
            #test if travelling down
            else:
        ...
        for sprite in collisionList:
            if sprite is self or type(sprite) is Bullet : continue
            #test if travelling to the right
            elif vect[1] > 0:
                self.rect.bottom = sprite.rect.top
            #test if travelling to the left
            else:
...
class Drops(pygame.sprite.Sprite):
    ...
    # Returns True if the bullet is to be destroyed otherwise it returns False
...
class Bullet(pygame.sprite.Sprite):
    ...
    def __init__(self, pos, size, rotation, ricochet, dieCollideBullet, dieCollideWall, creator):
        ...
        #position and player that created the bullet
        self.creator = creator
        self.pos = [pos[0], pos[1]]
        self.rotation = rotation
        
        #init the behaviour data
        self.ricochet = ricochet
        self.dieCollideBullet = dieCollideBullet
        self.dieCollideWall = dieCollideWall
        
        #init the image for Bullets
    ...
    # Authors: Tenn & Aniekan
    # @param wallSprites - a group of pygame sprite of type "Wall"
    # Returns False if the bullet is to be destroyed elseit returns True
    def ricochetBullet(self, wallSprites):
        ...
        if self.ricochet > 0 and len(collisionList) > 0:
            #calculate exit trajectory with each object that the player collided with
...
# Abstract class used to create the level    
# Author: Aniekan
# Course: ICS-3U
# Last Modified: Thurdsay, January 7, 2016
...
# Class used to transport Player and Bullet objects around the map
# Author: Aniekan
# Course: ICS-3U
# Last Modified: Saturday, January 9, 2016
...
class Main:
    ...
    def initLevel(self):
        ...
        #init the players and add them to the sprite lists
        ...
        # Creating Portal Objects using the portalParam list
        ...
        Rooms = []
        # Storing parmeters for the creating of walls (initialising level)
        ...
        Rooms.append(walls1)
        #create each wall with the position data and add it to the sprite lists
        for wall in walls1:
        ...
        for i in range(3):
            ...
            #loop until a drop is created where it does not collide with anything
            while newDrop.checkCol(self.allSprites) == True:
                newX = randrange(0,1200)
                newY = randrange(0,600)
                param[0] = (newX,newY)
                newDrop = Drops(param[0],param[1],param[2],param[3],param[4],param[5],param[6])
            #add drop to the sprite lists
    ...
    def eventHandler(self):
        ...
        for event in pygame.event.get():
            #test if event is Quit
            if event.type == pygame.QUIT:
                self.play = False
            #record all keydown events
            if event.type == pygame.KEYDOWN:
                #self.pressed2.append(chr(event.key))
                self.pressed[unichr(event.key)] = True
            #record all keyup events
            if event.type == pygame.KEYUP:
                self.pressed[unichr(event.key)] = False
