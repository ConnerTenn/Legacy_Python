...
#---------------------------------------------------------------------------
    ...
    def ResetData(self):
        ...
        Maze = DFS_maze_gen_final.Generate(self.MazeDimensions[0], self.MazeDimensions[1])
        self.Maze = [copy.deepcopy(Maze), copy.deepcopy(Maze)]
        ...
        self.RunnerListThreadCheck = []
        for I in range(0, self.PopulationSize):
            self.RunnerListThreadCheck.append(False)
...
#===============================================================================
    ...
    def CMD(self):
        ...
        if Selection == "2":
            ...
            self.CurrentObject[0] = int(input(">>"))
            self.State = "Testing"
            self.Test()
            self.State = ""
        if Selection == "3":
    ...
    def Simulate(self):
        ...
        self.ThreadNum = []
        for I in range(0, 2):
            self.ThreadNum.append(-1)
            Thread.append(threading.Thread(target=self.SimulateThread))
        for I in range(0, 2):
            Thread[I].start()
        for I in range(0, 2):
            Thread[I].join()
        
    def SimulateThread(self, Start=0, Step=1):
        I = 0
        ThreadNum = 0
        while I < len(self.ThreadNum):
            if self.ThreadNum[I] == -1:
                self.ThreadNum[I] = I
                ThreadNum = I
            I+=1
        #for Object in self.RunnerList:
        for self.CurrentObject[ThreadNum] in range(ThreadNum, len(self.RunnerList), len(self.ThreadNum)):
            self.ResetMazeData(-1)
            Object = self.RunnerList[self.CurrentObject[ThreadNum]]
            if self.PrintUpdates:
                print("Runner:" + str(self.CurrentObject[ThreadNum]+1) + "/" + str(self.PopulationSize) +\
                        " DNA Len:" + str(len(Object.DNA)) +" ")#, end="")
            #print(Object.DNA)
            Object.Run(self.MaxSteps, self.Maze[ThreadNum], self.SimulationDelay, self.MazeDimensions)
            self.ResetMazeData(-1)
            Object.GetFitness(self.Maze[ThreadNum], self.MazeDimensions, self.MaxSteps)
            if self.PrintUpdates:
    ...
    def Test(self):
        Object = self.RunnerList[self.CurrentObject[0]]
        if self.PrintUpdates:
            print("Runner:" + str(self.CurrentObject[0]+1) + "/" + str(self.PopulationSize) +\
                    " DNA Len:" + str(len(Object.DNA)), end="")
        #print(Object.DNA)
        Object.Run(self.MaxSteps, self.Maze[0], self.SimulationDelay, self.MazeDimensions)
        self.ResetMazeData(-1)
        Object.GetFitness(self.Maze[0], self.MazeDimensions, self.MaxSteps)
        if self.PrintUpdates:
            print(" Pos"+str(Object.Pos) +\
                    " Dist:" + str(Object.DistanceFromEnd) +\
                    " Fitness: " + str(Object.Fitness))
    ...
    def Kill(self):
        ...
        for self.CurrentObject[0] in range(0, len(self.RunnerList)):
            if random.sample(range(0, len(self.RunnerList)),1)[0] < self.CurrentObject[0]:
                self.RunnerList[self.CurrentObject[0]] = -1
    ...
    def Render(self):
        ...
        self.DrawMaze(self.Maze[0], [0,0])
        if self.State in ["Simulate"]:
            self.DrawMaze(self.Maze[1], [350,0])
            
        if self.State in ["Simulate", "Testing"]:
            self.RunnerList[self.CurrentObject[0]].Draw([0,0], self.PyGame, self.Screen)
        if self.State in ["Simulate"]:
            self.RunnerList[self.CurrentObject[1]].Draw([350, 0], self.PyGame, self.Screen)
        #  == End Draw Code ==
    ...
    def DrawMaze(self, Maze, Offset):
        for x in range(0, self.MazeDimensions[0]*10, 10):
            for y in range(0, self.MazeDimensions[1]*10, 10):
                ...
                if Maze[int(x/10)][int(y/10)][1] != -1 and self.State in ["Simulate", "Testing"]:
...
class Runner:
    ...
    def Draw(self, Offset, PyGame, Screen):
        x=self.Pos[0]
        y=self.Pos[1]
        if self.Direction%4 == 0:
            PyGame.draw.polygon(Screen, [0,0,255], [[Offset[0]+2+x*10, Offset[1]+10+y*10], [Offset[0]+5+x*10, Offset[1]+0+y*10], [Offset[0]+8+x*10, Offset[1]+10+y*10]])
        if self.Direction%4 == 1:
            PyGame.draw.polygon(Screen, [0,0,255], [[Offset[0]+0+x*10, Offset[1]+2+y*10], [Offset[0]+10+x*10, Offset[1]+5+y*10], [Offset[0]+0+x*10, Offset[1]+8+y*10]])
        if self.Direction%4 == 2:
            PyGame.draw.polygon(Screen, [0,0,255], [[Offset[0]+2+x*10, Offset[1]+0+y*10], [Offset[0]+5+x*10, Offset[1]+10+y*10], [Offset[0]+8+x*10, Offset[1]+0+y*10]])
        if self.Direction%4 == 3:
            PyGame.draw.polygon(Screen, [0,0,255], [[Offset[0]+10+x*10, Offset[1]+2+y*10], [Offset[0]+0+x*10, Offset[1]+5+y*10], [Offset[0]+10+x*10, Offset[1]+8+y*10]])
